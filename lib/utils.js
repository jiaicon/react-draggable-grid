...
export function resizeElement(
  ...
  return modifyLayout(layout, l);
}

export function recurrenceResizeEle(
  active=[],
  layout: Layout,
  old: LayoutItem,
  w: ?number,
  ...
  const l = getLayoutItem(layout, active[index]);
  if (!l || !old) return;
  // 宽、高、left按父级的百分比增加
  // const layout_resized = resizeElement(
  //   layout,
  //   l,
  //   w*l.w/old.w,
  //   h*l.h/old.h,
  ...
  const resize_item = {
    ...
    w: w*l.w/old.w,
    h: h*l.h/old.h,
    x: w*(l.x-old.x)/old.w + old.x + delX, // 先转化为相对于父级的比值，再加上父级的值
    y: h*(l.y-old.y)/old.h + old.y + delY,
  }
  const layout_resized = modifyLayout(layout, resize_item);
  // modifyLayout(layout, l) // 暂时去掉，当时的逻辑忘了
  index+=1;
  if (index <= active.length-1) {
    return recurrenceResizeEle(
      active,
      layout_resized,
      old,
      w,
...
export const recurrenceMoveEle = (
  active=[],
  layout: Layout,
  x: ?number,
  y: ?number,
  ...
  const l = getLayoutItem(layout, active[index]);
  if (!l) return;
  const layout_moved = moveElement(
    layout,
    l,
    x + l.x,
    y + l.y,
 ...
 * @param l: any[]
 */
export function clacBox(l: any[]) {
  let minX: undefined | number = undefined; // 最小的x轴
  let minY: undefined | number = undefined; // 最小的y轴
  let maxX: undefined | number = undefined; // 最大的x轴
  let maxY: undefined | number = undefined; // 最大的y轴
  // let maxW: undefined | number = undefined; // 最大的x轴项的宽度
  // let maxH: undefined | number = undefined; // 最大的y轴项的高度
  l.forEach(item => {
    if (minX === undefined || item.x < minX) {
      minX = item.x;
    }
    if (minY === undefined || item.y < minY) {
      minY = item.y;
    }
    if (maxX === undefined || item.x + item.w > maxX) {
      maxX = item.x + item.w;
      // maxW = item.w;
    }
    if (maxY === undefined || item.y + item.h > maxY) {
      maxY = item.y + item.h;
      // maxH = item.h;
    }
  })
